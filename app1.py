...
def check_duplicate_submission(student_id, subject, title, description, submission_type):
    ...
    # 1. Prevent any duplicate title for the same subject (regardless of date)
    res = supabase.table('submissions').select('submission_id, date, title').eq('student_id', student_id).eq('subject', subject).ilike('title', normalized_title).execute()
    if res.data:
        return True, f"You have already submitted an assignment with the same title. Please use a different title."

    # 2. Simple similarity check on title (word overlap) even without sklearn
    existing_titles = supabase.table('submissions').select('title').eq('student_id', student_id).eq('subject', subject).execute()
    if existing_titles.data:
        words_new = set(normalized_title.split())
        for ex in existing_titles.data:
            words_ex = set(ex['title'].lower().split())
            if words_new and words_ex:
                overlap = len(words_new & words_ex)
                union = len(words_new | words_ex)
                similarity = overlap / union if union > 0 else 0
                if similarity > 0.8:
                    return True, f"Your title is very similar to a previous submission '{ex['title']}'. Please use a more distinct title."

    # 3. AI similarity check for description (if sklearn is available and description long)
    if SKLEARN_AVAILABLE and len(description) > 100:
        recent = supabase.table('submissions').select('submission_id, description, date').eq('student_id', student_id).eq('subject', subject).execute()
        if recent.data:
            ...
            if len(texts) > 1:
                try:
                    ...
                    for i, sim in enumerate(similarities):
                        if sim > 0.85:
                            return True, f"⚠️ This submission is very similar ({sim:.1%}) to a previous submission. Please submit new work."
                except:
                    pass
    return False, ""
...
# ========== STUDENT SECTION ==========
# (same as your perfect working code – I'm keeping it exactly as you had it)
# ... [all student pages remain unchanged] ...
# For brevity, I'm not repeating them here, but you must keep them exactly as in your previous working version.
...
# ========== TEACHER SECTION ==========
    ...
    if st.session_state.page == "🏠 Teacher Dashboard":
        ...
        # ... (keep your existing subject management code – same as before) ...
        pass
